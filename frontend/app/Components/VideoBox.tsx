...
export default function VideoBox({ id }: any) {
  ...
  return (
    <div className='h-full rounded-lg'>
      {videoTrack && <video className='rounded-lg' autoPlay muted playsInline ref={videoElement} />}
      {audioTrack && <audio autoPlay className='rounded-lg' playsInline ref={audioElement} />}
    </div>
  );
}
